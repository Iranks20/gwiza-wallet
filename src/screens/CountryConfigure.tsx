...
const configureTabs = [
  ...
  { key: 'profile-type-groups', label: 'Profile Type Groups', path: 'profile-type-groups' },
...
export default function CountryConfigure() {
  ...
  const countryCode = (countryId ?? 'XX').toUpperCase()
  ...
  return (
    <div style={{ fontFamily: "'Poppins', sans-serif" }}>
      <Components.AdminPageHeader
        ...
        subtitle={`${countryName} — Configure limits, rules, fees and structures`}
      />

      {/* Country header summary: name / code / status */}
      <div className="flex flex-wrap items-center gap-4 mb-4 p-4 rounded-xl border" style={{ background: '#FFFFFF', borderColor: '#E5E7EB' }}>
        <div>
          <p className="text-xs mb-0.5" style={{ color: '#6B7280' }}>Name</p>
          <p className="font-semibold" style={{ color: '#04304B', fontSize: 15 }}>{countryName}</p>
        </div>
        <div>
          <p className="text-xs mb-0.5" style={{ color: '#6B7280' }}>Code</p>
          <p className="font-mono font-medium" style={{ color: '#37BBA2', fontSize: 14 }}>{countryCode}</p>
        </div>
        <div>
          <p className="text-xs mb-0.5" style={{ color: '#6B7280' }}>Status</p>
          <Components.StatusBadge status="active" size="sm" />
        </div>
      </div>
      ...
      <div className="mb-4 flex flex-wrap gap-2 border-b" style={{ borderColor: '#E5E7EB' }}>
        {configureTabs.map(t => {
          ...
          const active = path === to || (t.path !== 'profile-type-groups' && path.startsWith(to))
          return (
            <Link
              key={t.key}
              to={to}
